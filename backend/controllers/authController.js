...
export const registerUser = async (req, res) => {
  try {
    ...
    const profilePhotoPath = profilePhotos.length > 0 ? profilePhotos[0] : null;
    ...
    res
      .status(201)
      .json({ message: "Registration Successful", designation, email });
...
export const loginUser = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { email: user.email, userid: user.id, designation: user.designation },
