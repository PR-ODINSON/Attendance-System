    ...
    const fetchAttendance = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          const currentMonthData = data.filter(isCurrentMonthAttendance);
          const summary = {
            Present: currentMonthData.filter((item) => item.status === "Present").length,
            Absent: currentMonthData.filter((item) => item.status === "Absent").length,
            Late: currentMonthData.filter((item) => item.status === "Late").length,
            "Left Early": currentMonthData.filter((item) => item.status === "Left Early").length,
          };
          setAttendance(summary);
        } else {
          // No attendance data, default remains as 0
          setAttendance({ Present: 0, Absent: 0, Late: 0, "Left Early": 0 });
        }
      } catch (error) {
