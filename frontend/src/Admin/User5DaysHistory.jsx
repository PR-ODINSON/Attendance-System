...
const FiveDaysHistory = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          console.error(
            "Expected an array of attendance data, but got:",
            typeof data
          );
        }
      } catch (error) {
  ...
  const formatDate = (isoString) => {
    if (!isoString) return "—";
    const currentDate = new Date(isoString).toLocaleDateString("en-US", {
      ...
      <div className="w-full p-8 rounded-lg shadow flex justify-center items-start flex-col gap-4 cursor-pointer transition">
        ...
        <ul className="w-full flex flex-col gap-2 text-sm">
          {attendanceData.map((attendance) => (
            <div
              key={attendance.id}
              className="w-full bg-blue-50 flex justify-between items-center py-2 px-4 rounded-md hover:transition hover:scale-105 hover:bg-blue-100"
            >
